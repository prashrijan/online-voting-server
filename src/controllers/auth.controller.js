...
export const loginUser = async (req, res, next) => {
  try {
    ...
    if (!user.isVerified) {
      return res
        .status(400)
        .json(new ApiError(400, "Please verify your email before logging in."));
    }
...
export const googleAuthCallback = async (
  ...
  try {
    ...
    if (user && !user.googleId) {
      console.log("User exists but has no Google ID. Triggering failure.");
...
export const refreshToken = async (req, res) => {
  try {
    ...
    if (!newAccessToken) {
      return res
        .status(500)
        .json(new ApiError(500, "Failed to generate new access token."));
    }
