...
export const castVote = async (req, res, next) => {
  try {
    ...
    if (!election || election.status != "active") {
      return res.status(400).json(new ApiError(400, "Election is not active."))
    }

    if (!election.candidates.includes(candidateId)) {
      return res.status(400).json(new ApiError("Invalid Candidate for this election."))
    }

    const existingVote = await Vote.findOne({ userId, electionId })

    if (existingVote) {
      return res.status(400).json(new ApiError(400, "You have already voted for this election."))
    }

    const vote = await Vote.create({ userId, electionId, candidateId })

    return res.status(201).json(new ApiResponse(201, vote, "Vote casted successfully."))
  } catch (error) {
    console.error(`Internal Server Error : ${error}`)
    return next(new ApiError(500, "Server error casting vote to election."))
  }
}
...
export const getElectionResults = async (req, res, next) => {
  try {
    ...
    if (election.status === "active") {
      return res
        .status(404)
        .json(new ApiError(400, "Election is still acitve. Cannot get active election results."))
    }
    ...
    const results = await Vote.aggregate([
      {
        $match: {
          electionId: new mongoose.Types.ObjectId(electionId.toString()),
        },
      },
      {
        $group: {
      ...
      },
      {
        $project: {
          ...
          percentage: {
            $round: [
              {
                $multiply: [{ $divide: ["$voteCount", totalVotes] }, 100],
              },
              2,
            ],
          },
    ...
    const winners = resultsWithNames.filter((r) => r.voteCount === maxVotes)
